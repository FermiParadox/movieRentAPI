...
class User(Document):
    ...
    passphrase = StringField(min_length=12, max_length=70)
    balance = IntField(min_value=0, max_value=1000, default=0)
    rented_movies = DictField()


# ---------------------------------------------------------------------------
def validation_movie_categories(categories: List) -> None:
...
class Movie(Document):
    ...
    categories = ListField(validation=validation_movie_categories, required=True)
